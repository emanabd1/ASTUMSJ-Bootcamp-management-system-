...
const canAccessSession = (user, session) => user.role === "admin" || (user.role === "mentor" && (session.batch?.mentors || []).some((mentor) => sameId(mentor, user._id))) || (user.role === "student" && (session.batch?.students || []).some((student) => sameId(student, user._id)));
