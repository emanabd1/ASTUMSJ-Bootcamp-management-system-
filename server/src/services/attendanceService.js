...
async function canManage(user, studentId) {
  if (user.role === 'admin') {
    return true;
  }
...
async function summary(studentId, from, to) {
  ...
  return {
    ...
    applicableSessions: applicable,
    counts,
    attendancePercentage: applicable ? Number((counts.Present / applicable * 100).toFixed(2)) : 0
  };
}
