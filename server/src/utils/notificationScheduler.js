...
const notifyUpcomingDeadlines = async () => {
  ...
  const assignments = await Assignment.find({
    status: "active",
    deadline: { $gt: now, $lte: in48Hours }
  }).select("_id title deadline targetStudents");

  for (const assignment of assignments) {
    if (!assignment.targetStudents.length) {
      continue;
    }

    await Promise.all(
      assignment.targetStudents.map((studentId) =>
        Notification.updateOne(
          ...
          },
          {
            $setOnInsert: {
              ...
              message: `${assignment.title} is due on ${new Date(assignment.deadline).toLocaleString()}.`,
              type: "deadline",
...
const startNotificationScheduler = () => {
  const run = () =>
    notifyUpcomingDeadlines().catch((error) =>
      console.error("Deadline notification job failed:", error.message)
    );
