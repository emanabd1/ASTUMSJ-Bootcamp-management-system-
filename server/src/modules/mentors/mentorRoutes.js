...
router.get("/dashboard", async (req,res,next)=>{try{const students=await User.find({role:"student",mentor:req.user._id,status:"approved",isActive:true}).select("fullName email department yearOfStudy");const ids=students.map(s=>s._id);const [attendance,progress,pendingSubmissions,assignments,announcements]=await Promise.all([Attendance.find({student:{$in:ids}}),Progress.find({student:{$in:ids}}),Submission.find({student:{$in:ids},status:"submitted"}).populate("student","fullName").populate("assignment","title deadline"),Assignment.find({creator:req.user._id}).sort({createdAt:-1}).limit(10),Announcement.find({author:req.user._id}).sort({createdAt:-1}).limit(5)]);const studentStats=students.map(st=>{const a=attendance.filter(x=>String(x.student)===String(st._id));const p=progress.filter(x=>String(x.student)===String(st._id));const present=a.filter(x=>["Present","Late"].includes(x.status)).length;return {...st.toObject(),attendancePercentage:a.length?Math.round(present/a.length*100):0,progressCompleted:p.filter(x=>x.status==="Completed").length,progressTotal:p.length,atRisk:(p.length>0&&p.filter(x=>x.status==="Completed").length/p.length<0.5)||a.length>0&&present/a.length<0.75}});res.json({success:true,dashboard:{assignedStudents:studentStats,pendingGrading:pendingSubmissions,assignments,announcements,atRiskStudents:studentStats.filter(s=>s.atRisk)}})}catch(e){next(e)}});
...
router.get("/students", async (req, res, next) => {
  try {
    ...
    const enriched = await Promise.all(students.map(async (student) => {
      ...
      return { ...student.toObject(), attendancePercentage, progressCompleted: progress.filter(p => p.status === "Completed").length, progressTotal: progress.length, progressNeedsImprovement: progress.filter(p => p.status === "Needs Improvement").length };
    }));
