...
const { canManage, summary } = require("../../services/attendanceService");
...
router.get("/summary/:studentId", async (req, res, next) => {
  try {
    if (!(await canManage(req.user, req.params.studentId)) && req.user.role !== 'student') {
    ...
    res.json({
      success: true,
      summary: await summary(req.params.studentId, req.query.from, req.query.to)
    });
  } catch (e) {
    next(e);
  }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      res.status(201).json({ success: true, attendance: r });
    } catch (e) {
      next(e);
    }
  }
...
router.patch("/:id", authorize('admin', 'mentor'), async (req, res, next) => {
  try {
    const r = await Attendance.findById(req.params.id);
    if (!r) {
