...
const fs = require("fs");
// Fixed path to point to your assignment module where the model lives:
...
router.patch("/:id/grade", authorize("admin", "mentor"), async (req, res, next) => {
  try {
    ...
    if (submission.student && submission.student.mentor && !(await canManageSubmission(req.user, submission))) {
      return res.status(403).json({ success: false, message: "You can only view/grade submissions from assigned students." });
    }
    ...
    // Safely extract student ID for notification
    const targetStudentId = submission.student?._id || submission.student;

    if (!targetStudentId) {
      console.error("CRITICAL ERROR: Could not find student ID on submission for notification!");
    } else {
      try {
        ...
        console.log("✅ Notification successfully created in DB for Student ID:", notif.user);
      } catch (notifErr) {
