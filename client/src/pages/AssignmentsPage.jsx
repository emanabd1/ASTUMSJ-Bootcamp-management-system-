...
export default function AssignmentsPage(){
 ...
 const [submit,setSubmit]=useState({method:'github',githubUrl:'',liveDemoUrl:'',textAnswer:'',files:[]}); const [grade,setGrade]=useState({score:'',feedback:'',status:'graded'});
 ...
 const submitAssignment=async e=>{e.preventDefault();try{const fd=new FormData();fd.append('method',submit.method);fd.append('githubUrl',submit.githubUrl);fd.append('liveDemoUrl',submit.liveDemoUrl);fd.append('textAnswer',submit.textAnswer);[...submit.files].forEach(f=>fd.append('files',f));await axiosInstance.post(`/assignments/${selected}/submit`,fd,{headers:{'Content-Type':'multipart/form-data'}});setMessage('Assignment submitted.');open(selected)}catch(e){setMessage(e.response?.data?.message||'Could not submit.')}};
  ...
  {detail && <div className="fixed inset-0 z-50 bg-black/70 p-4 flex items-center justify-center" onClick={()=>setDetail(null)}>
   <div onClick={e=>e.stopPropagation()} className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-3xl border border-[#4a3b32] bg-[#1e1713] p-6">
    ...
    {user?.role==='student' ? <form onSubmit={submitAssignment} className="mt-6 space-y-3">
      ...
      <label className="block text-xs text-[#a39081]">Live demo URL (optional)<input className={field} placeholder="https://your-deployed-app.com" value={submit.liveDemoUrl} onChange={e=>setSubmit({...submit,liveDemoUrl:e.target.value})}/></label>
